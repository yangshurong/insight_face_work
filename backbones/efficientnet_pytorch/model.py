...
class EfficientNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x = self.extract_features(inputs)
        global_feats = self._avg_pooling(x)
        if self._global_params.include_top:
            global_feats = global_feats.flatten(start_dim=1)
            global_feats = self._dropout(global_feats)
            global_feats = self._fc(global_feats)
        return x, global_feats
